...
if __name__ == "__main__":
    ...
    #! Usage: python script_name.py FOLDER FILE [OUTPUT_FOLDER]
    ...
    if syb:
        input_path_name = os.path.join(input_folder, input_file)
        try:
            dumpfile(input_path_name, output_folder)
        ...
        input_path_name = os.path.join(input_folder, input_file)
        try:
            dumpfile(input_path_name, input_folder)
            print(f"Successfully converted: {input_file}")
        except Exception as e:
            print(f"Error converting {input_file}: {str(e)}")
